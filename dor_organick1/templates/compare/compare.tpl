...
<div id="content-wrapper" class="right-column col-xs-12 col-sm-12 col-md-12">
	<div id="dorcompare">
		<section id="products">
		<h1 class="h1 hidden">{l s='Product Comparison' d='Shop.Theme.Actions'}</h1>
		<h2 class="hidden">{l s='Product Comparison' d='Shop.Theme.Actions'}</h2>
		{if $hasProduct}
				...
				<table id="product_comparison" class="table table-bordered">
					<tr>
						<td class="td_empty compare_extra_information td-compare-header">
							...
							<span>{l s='Product:' d='Shop.Theme.Actions'}</span>
						</td>
						{foreach from=$products item=product name=for_products}
							...
							<td class="ajax_block_product comparison_infos product-block product-{$product.id_product}">
								<div class="remove">
									<a class="cmp_remove" href="#" title="{l s='Remove' d='Shop.Theme.Actions'}" data-id-product="{$product.id_product}">
								...
								<div class="product-image-block">
									...
									{if isset($product.new) && $product.new == 1}
										<a class="new-box box-status" href="{$product.url|escape:'html':'UTF-8'}">
											<span class="new-label">{l s='New' d='Shop.Theme.Actions'}</span>
										</a>
									{/if}
									{if isset($product.show_price) && $product.show_price && !isset($restricted_country_mode)}
										{if $product.on_sale}
											<a class="sale-box box-status" href="{$product.url|escape:'html':'UTF-8'}">
										...
										{if !(($product.quantity <= 0 && !$product.available_later) OR ($product.quantity != 0 && !$product.available_now) OR !$product.available_for_order)}
											...
											<span class="availability_value"{if $product.quantity <= 0} class="warning-inline"{/if}>
												{if $product.quantity <= 0}
													...
													{else}
														{l s='This product is no longer in stock.' d='Shop.Theme.Actions'}
													{/if}
												{else}
					...
					{else}
						<tr>
							<td></td>
							<td colspan="{$products|@count}" class="text-center">{l s='No features to compare' d='Shop.Theme.Actions'}</td>
						</tr>
					{/if}
		...
		{else}
			<p class="alert alert-warning">{l s='There are no products selected for comparison.' d='Shop.Theme.Actions'}</p>
		{/if}
		<ul class="compare_footer_link">
			<li>
				<a class="button lnk_view btn btn-default" href="{if isset($urls.force_ssl) && $urls.force_ssl}{$urls.base_url_ssl}{else}{$urls.base_url}{/if}">
					<span><i class="icon-chevron-left left"></i>{l s="Continue Shopping" d='Shop.Theme.Actions'}</span>
				</a>
			</li>
		</ul>
		</section>
