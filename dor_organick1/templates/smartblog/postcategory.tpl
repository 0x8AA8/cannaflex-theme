...
<div id="dor-smartblog-lists" class="center_column dor-two-cols col-xs-12 col-sm-9">
    ...
    {if $postcategory == ''}
        {if $title_category != ''}
             <p class="error">{l s='No Post in Category' d='Shop.Theme.Actions'}</p>
        {else}
             <p class="error">{l s='No Post in Blog' d='Shop.Theme.Actions'}</p>
        {/if}
    {else}
    ...
    <div id="smartblogcat" class="block row {$dorBlogsStyleCss}">
        ...
        {if isset($dorBlogsStyle) && ($dorBlogsStyle == 4 || $dorBlogsStyle == 5 || $dorBlogsStyle == 6) || (isset($smarty.get.type) && $smarty.get.type == 4)}
            {include file="./category_masonry.tpl" postcategory=$postcategory}
        {elseif (isset($dorBlogsStyle) && $dorBlogsStyle == 1) || (isset($smarty.get.type) && $smarty.get.type == 1)}
            {foreach from=$postcategory item=post key=i}
                {include file="./category_loop.tpl" postcategory=$postcategory}
            {/foreach}
        {elseif (isset($dorBlogsStyle) && $dorBlogsStyle == 2) || (isset($smarty.get.type) && $smarty.get.type == 2)}
            {foreach from=$postcategory item=post key=i}
                {include file="./category_loop_v2.tpl" postcategory=$postcategory}
            {/foreach}
        {elseif (isset($dorBlogsStyle) && $dorBlogsStyle == 3) || (isset($smarty.get.type) && $smarty.get.type == 3)}
            {foreach from=$postcategory item=post key=i}
                {include file="./category_loop_v3.tpl" postcategory=$postcategory}
            {/foreach}
        {else}
    ...
    <div class="post-page col-md-12">
			...
			<div class="col-md-6 hidden">
                <div class="results">{l s="Showing" d="Shop.Theme.Actions"} {if $limit_start!=0}{$limit_start}{else}1{/if} {l s="to" d="Shop.Theme.Actions"} {if $limit_start+$limit >= $total}{$total}{else}{$limit_start+$limit}{/if} {l s="of" d="Shop.Theme.Actions"} {$total} ({$c} {l s="Pages" d="Shop.Theme.Actions"})</div>
            </div>
  </div>
  </div> {/if}
 {/if}
