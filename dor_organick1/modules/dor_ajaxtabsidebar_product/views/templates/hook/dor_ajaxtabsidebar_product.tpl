...
<div id="dor-tabsidebar-product-category">
	<div class="dor-tabsidebar-product-category-wrapper">
		<ul role="tablist" class="nav nav-tabs dorTabAjaxSidebar" id="dorTabAjaxSidebar" data-ajaxurl="{if isset($urls.force_ssl) && $urls.force_ssl}{$urls.base_url_ssl}{else}{$urls.base_url}{/if}modules/dor_ajaxtabsidebar_product/productcategory-ajax.php">
			{$j=0}
		...
		{if $listTabsProduct}
			...
			<div aria-labelledby="cate-tab-data-{$productTab.id}-tab" id="cate-tab-data-{$productTab.id}" class="tab-pane fade {if $productTab.id==$tabID.id} active {/if} in">
				...
				<div class="view-all">
					<a class="btn btn-scale btn-go-cate" href="{$productTab.url}"><span><span class="fa fa-long-arrow-right">&nbsp;</span></span>{l s='View All' d='Modules.Dorajaxtabsidebar.Shop'}</a>
				</div>
			</div>
			{$k= $k+1}
			{/foreach}
		{/if}
			...
			<div aria-labelledby="cate-tab-data-{$cate.id}-tab" id="cate-tab-data-{$cate.id}" class="tab-pane fade {if $cate.id==$tabID.id} active {/if} in">
				...
				<div class="view-all">
					<a class="btn btn-scale btn-go-cate" href="#"><span><span class="fa fa-long-arrow-right">&nbsp;</span></span>{l s='View All' d='Modules.Dorajaxtabsidebar.Shop'}</a>
				</div>
			</div>
			{$key= $key+1}
			{/foreach}	
